...
def main():
    ...
    """
        ...
        to generate insightful business recommendations using OpenAI's GPT4-o Model 🚀 !!!
    """,
    ...
    if set(required_columns).issubset(df.columns):
        if not st.session_state.initial_recommendation_done:
            recommendation = generate_initial_recommendation(df)
        ...
        for chat_message in st.session_state.chat_history:
            # icon = "<i class='fa-solid fa-user'></i>" if chat_message["role"] == "user" else "<i class='fas fa-robot'></i>"
            # st.markdown(f"{icon} {chat_message['content']}", unsafe_allow_html=True)
