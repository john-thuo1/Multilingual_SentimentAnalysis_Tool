...
def validate_and_map_columns(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    if missing_columns:
        ...
        st.warning(
            "The dataset is missing some required columns. Please map the columns manually."
        )
...
def process_date_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df["Date"].isna().any():
        ...
        st.warning(
            f"{invalid_count} rows have invalid or unrecognized date formats. These rows will be ignored."
        )
...
def plot_overall_feelings(df: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure(
        go.Bar(x=counts.index, y=counts.values, width=0.3, marker=dict(color="purple"))
    )
    fig.update_layout(
...
def main() -> None:
    ...
    if len(selected_range) == 1:
        ...
        start_date, end_date = (
            pd.to_datetime(selected_range[0]),
            pd.to_datetime(selected_range[1]),
        )
        st.sidebar.info(
            f"Displaying data from {start_date.date()} to {end_date.date()}"
        )
    else:
    ...
    if "Sentiment Word Cloud" in selected_insights:
        ...
        sentiment_choice = st.sidebar.radio(
            "Select Sentiment", options=["Positive", "Negative", "Neutral"]
        )
